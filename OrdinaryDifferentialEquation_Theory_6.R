...
Figure.Group.1.Caption<-c("The following set of 4 figures show the relationships of",
                          ...
                          "")
Figure.1<-plot(t, temporal.sequence.1, type="l",xlab='phi',ylab='psi',main='',col='Blue')
legend("bottomright", legend=c(c(paste("mean:", mean(temporal.sequence.1), "\n var:", var(temporal.sequence.1))))
       , bty = "n",lwd=2, cex=0.75, lty=1:1)

Figure.2<-plot(t, temporal.sequence.2,  type="l", xlab='phi',ylab='psi',main='',col='Blue')
legend("bottomright", legend=c(c(paste("mean:", mean(temporal.sequence.2), "\n var:", var(temporal.sequence.2)))), bty = "n",lwd=2, 
       cex=0.75, lty=1:1)
...
Figure.1<-plot(x, cubic.drift.equation(x,1,1,1,1), xlab='phi',ylab='psi',main='',col='Blue')
legend("topright", legend=c(""), bty = "n",lwd=2, cex=0.75, lty=1:1)

Figure.2<-plot(x, quadratic.diffusion.equation(x,1,0,1), xlab='phi',ylab='psi',main='',col='Green')
legend("topright", legend=c(""), bty = "n",lwd=2, cex=0.75, lty=1:1)

Figure.3<-plot(x, cubic.drift.equation(x,0,0,-1,0), xlab='phi',ylab='psi',main='',col='Blue')
legend("topright", legend=c(""), bty = "n",lwd=2, cex=0.75, lty=1:1)
...
#---------------------------------------Function Library------------------------
Function.Caption<-c("Equation 1 shows cubic polynomial.",
                    "Equation 2 shows quadratic polynomial")
